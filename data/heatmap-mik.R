#' heatmap.mik
#'
#' This function extends the functionality of the heatmap.2 function in the R package gplots: \cr
#' \cr
#' https://cran.r-project.org/web/packages/gplots/index.html \cr
#' \cr
#' Allows additional rows of annotation to be plotted above the heatmap.
#' Adding row names to the colour matrix will display these alongside the colour rows in the heatmap.
#' See the manual page for the gplots heatmap.2 function for more information.
#' @param x is a gene expression data matrix: rows are genes, columns are samples.
#' @param ColSideColors is a matrix of colours for display above the columns of the heatmap.
#' @keywords heatmap, heatmap.2, gplots
#' @export
#' @examples
#' heatmap.mik()

# See edits below with "MIK" tag to see what has been changed in original heatmap.2 code
# It is VERY little.  Should just be able to copy those lines across to update new
# heatmap.2 code from gplots.

# Based on gplots 3.0.1.1

heatmap.mik <- function (x,
...
)
{
  ...
  if( missing(lmat) || is.null(lmat) )
  {
    ...
    if(!missing(ColSideColors)) { ## add middle row to layout
      ## MIK: edit to allow matrix for ColSideColors
  ...
  if(!missing(ColSideColors)) {
    ## MIK: edits to plot color matrix
    ## Begin edit
    if(is.null(nrow(ColSideColors))) {
    ## End edit
  ...
  ## MIK: the function invalid() doesn't exist?
  ...
  {
    flag <- try(
      ## MIK: plot.dendrogram no longer able to be accessed in this way?
