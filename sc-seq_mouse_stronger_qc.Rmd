---
title: "sc-seq_mouse_stronger_qc"
...
# Working in the same directory as sc-seq_mouse.Rmd, can keep object names the same but make sure to label outputs differently (and put them in the right output folder)
...
# Pre-process the data (as done by Conor)
```{r}
# # Load the CellRanger output
# # mouse.data <- Read10X(data.dir = "...")
# # Initialize the Seurat object with the raw (non-normalized data)
# mouse.dat <- CreateSeuratObject(counts = mouse.data, project = "druggedMice")
# # Inspect the data
# mouse.dat
# 
...
# 
# # SaveRDS(mouse.dat, "~/R/sc-seq_mouse/output/scDrugMiceAll_seurat.RDS")
```

# Load and further annotate the data
```{r}
# Load data file: output from CellRanger, already loaded in and converted to a Seurat object, and with Idents already renamed to sample names as above.
mouse.dat <- readRDS("~/R/sc-seq_mouse/output/scDrugMiceAll_seurat.RDS")
...
treat.ident <- as.factor(treat.ident)
mouse.dat@meta.data[["treat.ident"]] <- treat.ident
...
samp.ident.desc <- as.factor(samp.ident.desc)
mouse.dat@meta.data[["samp.ident.desc"]] <- samp.ident.desc
...
counts.vs.features <- FeatureScatter(mouse.dat, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
counts.vs.features
...
features.vs.percentmt <- FeatureScatter(mouse.dat, feature1 = "nFeature_RNA", feature2 = "percent.mt")
features.vs.percentmt

# Try and plot some of the potential thresholds on the data to see what you're removing:
...
# View the data again
counts.vs.percentmt <- FeatureScatter(mouse.dat, feature1 = "nCount_RNA", feature2 = "percent.mt", fill.by = "high.percent.mt")
counts.vs.percentmt
# Of course, now that the scale is tighter you could also decide to cut down to 7.5% or 8% mt reads

# Filtering accounting for doublets (kinda) and over-representation of mitochondrial DNA. Looking at the data, a good arbitrary threshold might be 10%
mouse.dat <- subset(mouse.dat, subset = nFeature_RNA > 200 & nFeature_RNA < 7500 & percent.mt < 10)
# These thresholds look pretty good. The problem with the statistically groovy ones above is that they don't actually cut off any samples with low features or counts, as 3 MADS less than the median is negative. You could try using those and then also these ones?
# When looking at the thresholds to cut off high counts and features (i.e. excluding doublets) these may also be kind of low. For the nFeature threshold especially, it looks like you might just be chucking out some interesting cells that aren't necessarily doublets.
# Check these with Mik
```

# Normalisation and subsetting
```{r}
# Using default settings
mouse.dat <- NormalizeData(mouse.dat, normalization.method = "LogNormalize", scale.factor = 10000)

# Now that the data is normalised, in future you should subset it from here
# das.dat <- subset(mouse.dat, subset = ) etc...
```
...
variable.genes <- VariableFeaturePlot(mouse.dat)
variable.genes.lab <- LabelPoints(plot = variable.genes, points = top10, repel = TRUE)
variable.genes.lab
```
...
mouse.dat <- FindNeighbors(mouse.dat, dims = 1:16)
...
mouse.dat <- RunUMAP(mouse.dat, dims = 1:16)
...
# png(file ="output/Heatmap_qc.png", width = 4000, height = 4000)
...
# Attempt to assign some cell types automatically based on the marker genes
```{r}
...
```
##... and from here we're working on creating and assigning new clusters and cell types

# try and identify tumour cells
```{r}
# Subset the Seurat object to include only the four dasatinib samples
das.dat <- subset(mouse.dat, subset = treat.ident == c("Das - drug", "Das - control"))


# Add treatment identifiers to the data
treat.ident.plain <- das.dat@meta.data[["samp.ident"]] %>% as.character(.)
treat.ident.plain[treat.ident.plain %in% "BOMT1"]  <- "Dasatinib 1"
treat.ident.plain[treat.ident.plain %in% "BOMT2"]  <- "Dasatinib 2"
treat.ident.plain[treat.ident.plain %in% "BOMC1"]  <- "Control 1"
treat.ident.plain[treat.ident.plain %in% "BOMC2"]  <- "Control 2"

treat.ident.plain <- as.factor(treat.ident.plain)
das.dat@meta.data[["treat.ident.plain"]] <- treat.ident.plain

# Since you're doing this down here, we have all of the previously annotated info already in the object. This is to plot the clusters determined previously...
DimPlot(das.dat, reduction = "umap", label = TRUE, repel = TRUE, group.by = "type.sctype")
```

# Quick Side-bar - determine if das only cluster 8 still exists in the same way with the same markers between das treated and all control samples
```{r}
# Find cluster markers to see if the same cluster is still present in the full dataset including the other control samples
# Subset data
das_vs_control_dat <- subset(mouse.dat, subset = treat.ident == c("Das - drug", "Das - control", "Ent - control", "Mir - control"))

# Dimensions
das_vs_control_dat <- JackStraw(das_vs_control_dat, num.replicate = 100)
das_vs_control_dat <- ScoreJackStraw(das_vs_control_dat, dims = 1:20)



JackStrawPlot(das_vs_control_dat, dims = 1:20)
ElbowPlot(das_vs_control_dat, ndims = 50)
das_vs_control_dat <- FindNeighbors(das_vs_control_dat, dims = 1:16)
das_vs_control_dat <- FindClusters(das_vs_control_dat, resolution = 0.6)
das_vs_control_dat <- RunUMAP(das_vs_control_dat, dims = 1:16)
DimPlot(das_vs_control_dat, reduction = "umap", label = T)
DimPlot(das_vs_control_dat, reduction = "umap", split.by = "treat.ident", label = T)
DimPlot(das_vs_control_dat, reduction = "umap", group.by = "samp.ident")
DimPlot(das_vs_control_dat, reduction = "umap", split.by = "samp.ident", label = T)

# Muc13 and Dmbt1, named markers for OG cluster 8
DimPlot(das_vs_control_dat, reduction = "umap", label = T, split.by = "samp.ident.desc")
FeaturePlot(das_vs_control_dat, features = "Muc13", split.by = "samp.ident.desc")
FeaturePlot(das_vs_control_dat, features = "Dmbt1", split.by = "samp.ident.desc")
...
FeaturePlot(das_vs_control_dat, features = "Akp3", split.by = "samp.ident")
FeaturePlot(das_vs_control_dat, features = "Akp3", split.by = "samp.ident")
# Ada and Adh6a, top third and fourth markers for cluster 8
FeaturePlot(das_vs_control_dat, features = "Ada", split.by = "samp.ident")
FeaturePlot(das_vs_control_dat, features = "Adh6a", split.by = "samp.ident")

# Find das_vs_control_dat markers for cluster 3, which seems similar to cluster 8:
das_vs_control.markers.3 <- FindMarkers(das_vs_control_dat, ident.1 = 3) %>% .[order(.$avg_log2FC, decreasing = TRUE ),]
write.csv(das_vs_control.markers.3, file = "output_qc/das_vs_control_markers_3_qc.csv")

png(file = "output_qc/das_vs_all_controls_qc.png", width = 4000, height = 1000, units = "px")
DimPlot(das_vs_control_dat, reduction = "umap", label = T, split.by = "samp.ident.desc")
dev.off()
...
# Looking at the ElbowPlot, since the JackStrawPlot is confusing, looks like about 16 PCs is pretty good
# More notes on these methods can be found in the Seurat guide
```
...
```{r}
# Construct a K nearest neighbours graph based on the distance between points in PCA space and refine edge weights between cells with Jaccard similarity. Using the first 16 PCs here.
das.dat <- FindNeighbors(das.dat, dims = 1:16)
...
das.dat <- RunUMAP(das.dat, dims = 1:16)
DimPlot(das.dat, reduction = "umap", label = T)
DimPlot(das.dat, reduction = "umap", split.by = "treat.ident", label = T)
DimPlot(das.dat, reduction = "umap", group.by = "samp.ident")
DimPlot(das.dat, reduction = "umap", split.by = "samp.ident", label = T)
```
...
# Scater is deprecated, use gridExtra::gridarrange instead
# scater::multiplot(DimPlot(das.dat, reduction = "umap", label = TRUE, repel = TRUE, cols = ccolss), gggr, cols = 2)

# Use gridExtra to print these graphs next to each other, with the UMAP annotated with original cluster ID's, not annotated cell types
png(filename = "output_qc/type.sctype_vs_celltype_qc.png", width = 2000, height = 1000, units = "px")
gridExtra::grid.arrange(DimPlot(das.dat, reduction = "umap", label = TRUE, repel = TRUE, cols = ccolss, group.by = "type.sctype"), gggr, ncol = 2)
dev.off()
DimPlot(das.dat, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'type.sctype')
...
# write(das.markers.8.pos, "output_qc/das.markers.8.pos.txt")
...
#   filter(cluster == 8) %>% .$gene
# write(das.markers.8.neg, "output_qc/das.markers.8.neg.txt")
...
# Cdh1
FeaturePlot(das.dat, features = "Cdh1", split.by = "treat.ident.plain", shape.by = "isMUC13_DMBT1pos")
#Trp53
FeaturePlot(das.dat, features = "Trp53", split.by = "treat.ident.plain", shape.by = "isMUC13_DMBT1pos")
# Cd44 (since Cdh1/Trp53 KO cells should be expressing Cd44
FeaturePlot(das.dat, features = "Cd44", split.by = "treat.ident.plain", shape.by = "isMUC13_DMBT1pos")
# Krt14, which is underexpressed with Cdh1 loss according to a paper from the lab
FeaturePlot(das.dat, features = "Krt14", split.by = "treat.ident.plain", shape.by = "isMUC13_DMBT1pos")
# -------------------------------------------------------------------------------------------------------------
...
# Let's also look at Cdh1 and Trp53 - If we're looking at cancer cells (but only the responding ones) in cluster 8, we'd hope these genes weren't expressed strongly there
# Cdh1
VlnPlot(das.dat, features = "Cdh1",  group.by = "type.manual.v1")
#Trp53
...
VlnPlot(das.dat, features = c("Lgals4", "Akr1B10", "Reg4"), group.by = "custom_classif")
...
# VlnPlot
VlnPlot(das.dat, features = c("Cdh1", "Trp53", "Cd44", "Dmbt1", "Muc13"),  group.by = "type.manual.v1")
...
VlnPlot(das.dat, features = "Msmb", group.by = "type.manual.v1")
...
png("output_qc/annotedclusterids_qc.png", height = 1000, width = 2000, units = "px")
...
png("output_qc/VlnPlotCdh1_qc.png", height = 1000, width = 2000, units = "px", res = 0.4, pointsize = 300)
...
jpeg("output_qc/VlnPlotCdh1_qc.jpeg", height = 1000, width = 2000, units = "px")
VlnPlot(das.dat, features = "Cdh1") + NoLegend()
dev.off()

png("output_qc/annotedclusteridsall_qc.png", height = 1000, width = 2000, units = "px", pointsize = 20)
DimPlot(das.dat, reduction = "umap", label = TRUE, pt.size = 0.8, group.by = "type.manual.v1", split.by = "samp.ident.desc") + NoLegend()
dev.off()

png("output_qc/type.sctype_qc.png", height = 1000, width = 2000, units = "px", pointsize = 20)
...
# This chunk isn't a true differential expression analysis, but does give you some idea of what genes are different between treatment and control
* This workflow was taken from here: https://satijalab.org/seurat/archive/v3.1/immune_alignment.html
```{r}
# Load packages
library(Seurat)
library(SeuratData)
...
# Work with example code from Seurat:
InstallData("ifnb")
data("ifnb")
# Split Seurat Object into list containing a seurat object for each treatment category
...
# Identify anchors to integrate datasets together
...
# Shouldn't need to start das.dat from zero or anything, I think it's fine to "re-normalise" it since the split object doesn't seem to contain any clustering or anything like that.
# Keeping number of PCs and res the same as previous das analysis for reproducibility
# Split Seurat Object into list containing a seurat object for each treatment category
das.dat.list <- SplitObject(das.dat, split.by = "treat.ident")
# Normalise data?
das.dat.list <- lapply(X = das.dat.list, FUN = function(x) {
    x <- NormalizeData(x)
    x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 2000)
})

# Identify anchors to integrate datasets together
das.anchors <- FindIntegrationAnchors(object.list = das.dat.list, dims = 1:16)
das.combined <- IntegrateData(anchorset = das.anchors, dims = 1:16)

DefaultAssay(das.combined) <- "integrated"

# Run the standard workflow for visualization and clustering
das.combined <- ScaleData(das.combined)
das.combined <- RunPCA(das.combined)
# t-SNE and Clustering
das.combined <- RunUMAP(das.combined, dims = 1:16)
das.combined <- FindNeighbors(das.combined, dims = 1:16)
das.combined <- FindClusters(das.combined, resolution = 0.6)

# Visualization
p1 <- DimPlot(das.combined, reduction = "umap", group.by = "treat.ident")
p2 <- DimPlot(das.combined, reduction = "umap", label = TRUE)
plot_grid(p1, p2)
DimPlot(das.combined, reduction = "umap", split.by = "treat.ident", label = T)
DimPlot(das.combined, reduction = "umap", split.by = "samp.ident", label = T)
# Ah-ha! We still have a cluster of cells that's different between treatment and control, except it's now cluster 12 (and it's also preserved in all samples as only in the two drugged ones)

# Identify conserved (across conditions) canonical cell markers: Uses DE gene expression for each dataset/group and combines p-values using meta-analysis methods from the MetaDE R package
DefaultAssay(das.combined) <- "RNA"
# Find conserved markers (i.e. for a cell type between both datasets. However, for some reason can only do on one cell type at a time)
das.combined.12.markers <- FindConservedMarkers(das.combined, ident.1 = 12, grouping.var = "treat.ident")
View(das.combined.12.markers)
# This provides a warning that the thing I just did was kind of pointless since I can only find that cluster in one treatment type anyway
# Interestingly it also provides a similar list (once sorted by avg log2FC) to my original cluster 8 data, but with a slightly different order of marker genes lower down




# Can next try to find markers normally? or just move on to trying to annotate clusters across both cell types, but sctype might get confused by the list, rather than Seurat object. Save this for Sunday...
```
# To try?
...
###################################### ANY OF THE PSEUDO-BULKING FROM HERE ON REFERS TO type.manual.v1, SO MAKE SURE YOU'RE USING THE CORRECT LABELS EVERYTIME YOU WANT TO FIND DE GENES FOR A CLUSTER
...
```{r}
# Define the cell type you want to extract
cluster.type <- "Lymphatic endothelial cells"
...
capture.output(DE.Cdh1.neg, file = "output_qc/DE.Cdh1.neg_qc.csv")
```
...
write.csv(rownames(sig.tt.epithelial.all)[sig.tt.epithelial.all$logFC < 0], file = "output_qc/epithelial_down_qc.csv")
write.csv(rownames(sig.tt.epithelial.all), file = "output_qc/epithelial_up_down_qc.csv")


write.csv(rownames(sig.tt.epithelial.all)[sig.tt.epithelial.all$logFC > 0], file = "output_qc/epithelial_up_qc.csv")
```
