---
title: "2_sc-seq_dasatinib_subclustering"
...
```{r}
das.dat <- readRDS("~/R/sc-seq_mouse/2_output/das_dat_processed.RDS")
```

### Load all data from Conor
```{r}
# CV.all.dat <- readRDS("~/R/sc-seq_mouse/data/CV_mice_dat_annotated.RDS")
```
...
```{r}
# Found notes online indicating to re-run everything except NormalizeData, as the data is already normalised (in the "data" element of the SeuratObject)
# das.dat.epithelial <- NormalizeData(object = das.dat.epithelial)
...
# Developing a theory
# Clusters 3 and 4 are very similar, definitely progenitor cells. However, cluster 4 has some quite different gene expression. This may be because it's going through the cell cycle (it expresses many of Dong et al 2023's markers for this, Top2a and Mki67), but it could also be developing early cancer.
# Foxm1 stimulates progression through the cell cycle (Takada et al. 2023), but also may have a role in EMT in GC (Miao et al. 2014), upregulating Zeb1, Zeb2, Vim, and down-regulating Cdh1. There's some evidence that Foxm1 is involved in epithelial progenitor expansion leading to cancer (Gemenetzidis et al. 2011)
# Same paper also suggest Foxm1 down = NF-κB p65 subunit, cyclin D1, Hes-1, VEGF, and EpCAM protein down.
# Nothing is going on in the hedgehog pathway (Shh, Ihh, Hh), despite some of the literature around this.
# Twist1 isn't strongly expressed anywhere, despite some evidence that it upregulates Foxm1 in GC https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0077625
# Some evidence that Foxm1 upregulates Ctsd, which then downregulates Cdh1, but there's no Ctsd expression in cluster 4 here https://pubmed.ncbi.nlm.nih.gov/28978107/
# CLuster 4 also expresses high amounts of Top2a and Mki67 compared to other cells
# In our data, Zeb1 and Zeb2 are pretty unclear, but there's definitely more vimentin. Vimentin is the major cytoskeletal component of mesenchymal cells, but also is just in mitosis?
...
# After talking to Sefi Rosenbluh at genemappers, look at some genes related to the Yap/Hippo and b-Cat/Wnt pathways
...
# From Sefi's paper b-Catenin-Driven Cancers Require a YAP1 Transcriptional Complex for Survival and Tumorigenesis
...
# Genes from Chella and her paper
VlnPlot(das.dat.epithelial, features = c("Cd44", "Sfrp2", "Thy1", "Thbs4", "Il1r1"), split.by = "treat.ident") + plot_annotation(title = "Chella genes")
VlnPlot(das.dat.epithelial, features = c("Cxcl14", "Il1r2", "Oasl1", "Fgf8", "Adm", "Hpgd", "Apoa4", "Mef2c"), split.by = "treat.ident") + plot_annotation(title = "Chella genes up in SRCC vs poorly differentiated")
VlnPlot(das.dat.epithelial, features = c("Ceacam3", "Ceacam6"), split.by = "treat.ident") + plot_annotation(title = "Chella genes up in poorly differentiated vs SRCC")
...
### Plots with median lines and Sefi's genes by Cdh1 expression, as well as some misc stuff
```{r}
# Ensure active idents are correct
Idents(das.dat.epithelial) <- das.dat.epithelial@meta.data[["seurat_clusters"]]

# Code that adds a line at the median for violin
VlnPlot(das.dat.epithelial, features = c("Cdh1"), split.by = "treat.ident") + stat_summary(fun = median, geom = "crossbar", 
                                                                                           position = position_dodge(width = 0.9), 
                                                                                           width = 0.25, color = "black", linewidth = 0.5)

# Look at the expression of Wnt/B-catenin related genes by Cdh1 expression level
Idents(das.dat.epithelial) <- das.dat.epithelial@meta.data[["cdh1.expression"]]

# Sefi genes by treat identity
VlnPlot(das.dat.epithelial, features = c("Ccn1", "Ccn2", "Axin2"), split.by = "treat.ident") + stat_summary(fun = median, geom = "crossbar", 
                                                                                                            position = position_dodge(width = 0.9), 
                                                                                                            width = 0.25, color = "black", linewidth = 0.5)

# Sefi genes just by Cdh1 expression
VlnPlot(das.dat.epithelial, features = c("Ccn1", "Ccn2", "Axin2")) + stat_summary(fun = median, geom = "crossbar", 
                                                                                  position = position_dodge(width = 0.9), 
                                                                                  width = 0.25, color = "black", linewidth = 0.5)
...
Idents(das.dat.epithelial) <- das.dat.epithelial@meta.data[["type.manual.v1"]]
# EMT:
VlnPlot(das.dat.epithelial, features = c("Foxm1", "Cdh1", "Cdh2", "Zeb1", "Zeb2", "Vim"))
# These are markers of EMT from google: negative (Cdh1 and cytokeratins) and positive (vimentin, N-cadherin (Cdh2), fibronectin), plotted alongside Foxm1 for comparison.



# Interesting features/cancer stem cell markers/cancer markers but split up a bit so they aren't impossible to read
VlnPlot(das.dat.epithelial, features = c("Lama3", "Lamb2", "Itga2", "Itga3"), split.by = "treat.ident")
VlnPlot(das.dat.epithelial, features = c( "Lrig1", "Foxm1", "Sox2", "Clu", "Ptk2"))
VlnPlot(das.dat.epithelial, features = c( "Prom1", "Icam1", "Epcam", "Aldh1a1", "Aldh1a3", "Klf4"))
# HDGC genes from https://aacrjournals.org/cancerpreventionresearch/article-abstract/17/8/361/746638/Stool-Protein-Mass-Spectrometry-Identifies (identified because ltf is near the top of the list in our dataset)
VlnPlot(das.dat.epithelial, features = c("Ltf", "Tpm2", "Dpp4", "Vcp", "Asah2", "Pip"))
VlnPlot(das.dat.epithelial, features = c("Gli1", "Hsp90ab1", "Lefty1"))
VlnPlot(das.dat.epithelial, features = c("Ptk2"), split.by = "treat.ident")
...
# IS THIS TREATMENT PER CELL? looks like it's just the ID for the treatment type repeated, without any actual info on which cell is which
...
for(x in levels(das.dat.epithelial@meta.data[["type.manual.v1"]])){
  ...
  design.x %>% tail()
  # Fit linear model
  # voom plot to see what it looks like - pretty weird, but you don't actually use the voom output here since allegedly it's already normalised
...
for(x in levels(das.dat.epithelial@meta.data[["type.manual.v1"]])){
  ...
  # Remove duplicates
  # ranks.x.v2 <- ranks.x.v2[!duplicated(names(ranks.x.v2))]
  ...
  # names(gsea.epithelial.limma)[x] <- names(tt.epithelial.limma)[x]
  # To make a version without filtering on gene set maximum and minimum sizes
  # gsea.epithelial.limma[[x]] <- fgsea(gsea.reactome.x, ranks.x.v2) %>% .[order(padj),] %>% .[.$padj < 0.05,]
  ...
  # names(gsea.epithelial.limma.collapsed)[x] <- names(tt.epithelial.limma)[x]
}
...
for(y in names(cts.final)){
  # # Filter out genes that fail to show at least 1 read per million
  # keep.y <- rowSums(edgeR::cpm(cts.final[[y]])>1) >= 1
...
for(y in names(tt.epithelial.limma.pseudo)){
  ...
  # Remove duplicates
  # ranks.y.v2 <- ranks.y.v2[!duplicated(names(ranks.y.v2))]
  
  # Test for significant pathway over-representation
  gsea.epithelial.limma.pseudo[[y]] <- fgsea(gsea.reactome.y, ranks.y.v2, maxSize = 500, minSize = 20) %>% .[order(padj),] %>% .[.$padj < 0.05,]
  # To make a version without filtering on gene set maximum and minimum sizes
  # gsea.epithelial.limma.pseudo[[y]] <- fgsea(gsea.reactome.y, ranks.y.v2) %>% .[order(padj),] %>% .[.$padj < 0.05,]
...
for(z in names(tt.epithelial.DESeq2.pseudo)){
  ...
  # Remove duplicates
  # ranks.z.v2 <- ranks.z.v2[!duplicated(names(ranks.z.v2))]
  # Test for significant pathway over-representation
  gsea.epithelial.DESeq2.pseudo[[z]] <- fgsea(gsea.reactome.z, ranks.z.v2, maxSize = 500, minSize = 20) %>% .[order(padj),] %>% .[.$padj < 0.05,]
  # To make a version without filtering on gene set maximum and minimum sizes
  # gsea.epithelial.DESeq2.pseudo <- fgsea(gsea.reactome.z, ranks.z.v2) %>% .[order(padj),] %>% .[.$padj < 0.05,]
  # Create a list of pathways with children collapsed
...
for(a in names(gsea.epithelial.DESeq2.pseudo.collapsed)){
  ...
  # Find out which pathways are present in 
  # intersect.a <- Reduce(intersect, list(a.1, a.2, a.3))
  # # Print the intersection out
  # cat(paste0(a, " Common Parent Pathways:"), paste(intersect.a, collapse = ", "), "\n")
  ...
  # # Print the intersection for this iteration
  # if (length(intersect.a) > 0) {
  #   cat(paste0(a, " Common Parent Pathways (3):"), paste(intersect.a, collapse = ", "), "\n")
  # } else {
  #   cat("Iteration", a, "no common elements.\n")
  # }  
}
...
for(b in names(gsea.epithelial.limma.pseudo)){
  ...
  # gsea.overlap.bbb <- gsea.overlap.bbb %>% dplyr::mutate(across(where(is.numeric), ~ round(.x, 4)))
...
for(b in names(gsea.epithelial.limma)){
  ...
  # gsea.epithelial.limma.table[[b]] <- gsea.epithelial.limma.table[[b]][order(gsea.epithelial.limma.table[[b]]$`Reactome Pathway Term`), ]
...
for(b in names(gsea.epithelial.limma.pseudo)){
  ...
  # Sort by pathway alphabetically
  # gsea.epithelial.limma.pseudo.table[[b]] <- gsea.epithelial.limma.pseudo.table[[b]][order(gsea.epithelial.limma.pseudo.table[[b]]$`Reactome Pathway Term`), ]
  # Sort by adjusted P-value
...
print(p)
# ggsave(filename = paste0(i, "_expression_violin_cancer_vs_all.png"), p, dpi = 300, width = 3000, height = 2000, units = "px")
...
# Large list
#pathways <- c("The citric acid (TCA) cycle and respiratory electron transport", "Respiratory electron transport", "Beta-catenin independent WNT signaling", "RHO GTPase Effectors", "MAPK family signaling cascades", "Negative regulation of MAPK pathway", "Programmed Cell Death", "Autophagy", "MAPK6/MAPK4 signaling", "Macroautophagy", "Selective autophagy", "MAPK1/MAPK3 signaling", "RAF/MAP kinase cascade", "Complex I biogenesis", "Signaling by WNT", "RAF activation", "MAPK targets/ Nuclear events mediated by MAP kinases", "Pyruvate metabolism and Citric Acid (TCA) cycle", "MTOR signalling", "EGFR downregulation")
# List for thesis
...
```{r}
# Create a summary table of DE results for thesis
# Need a column of number of DE genes
# Need a column of DE genes
# Need a column of number of fgsea Reactome pathways and collapsed pathways (for visualisation)
# Need a column of fgsea Reactome pathways
